    ...
    try {
        ...
        const sidebarHTML = `
            ...
            <nav class="flex-grow space-y-2">
                ...
                ${(userData.isAdmin || (userData.badge === '101' && userData.department === 'WSP') || (userData.rank === 'Sheriff' && userData.department === 'OCSO')) ? `
                <div>
    ...
    const showSuggestions = () => {
        ...
        if (filtered.length > 0) {
            filtered.slice(0, 5).forEach(citizen => {
                ...
                div.addEventListener('mousedown', (e) => { e.preventDefault(); inputElement.value = `<@${citizen.discordId}>`; suggestionsContainer.classList.add('hidden'); });
